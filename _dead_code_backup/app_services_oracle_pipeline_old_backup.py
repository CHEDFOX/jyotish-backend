...
from .user_memory import recall_user_memory, store_user_memory
...
class OraclePipeline:
    ...
    def _complete_pipeline(self, user_message: str, birth_data: Dict,
        ...
        # Step 2.5: Load user memory (silent, never exposed)
        memory = None
        memory_prompt = ""
        if birth_data:
            try:
                memory = load_user_memory(birth_data)
                if memory:
                    memory_prompt = build_memory_prompt(memory)
            except Exception:
                pass  # memory is optional — never break the pipeline

        # Step 3: CONSULTATION ENGINE (replaces basic assemble_data)
        ...
        # Step 4: Build prompt (with memory injected)
        system_prompt = build_oracle_prompt(intent, data_packet, user_message, memory_prompt)
...
def store_oracle_memory(birth_data, user_message, oracle_response, hook, intent):
    """Call AFTER the Oracle responds to store memory."""
    try:
